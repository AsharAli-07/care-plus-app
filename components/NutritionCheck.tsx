...
const MEAL_META: Record<MealKey, { emoji: string; time: string }> = {
  breakfast: { emoji: "🌅", time: "Morning" },
  lunch:     { emoji: "☀️", time: "Afternoon" },
  dinner:    { emoji: "🌙", time: "Evening" },
};
  ...
  <View style={s.card}>
    ...
    <View style={s.mealRow}>
      {mealKeys.map(key => {
        ...
        const active = meals[key];
        return (
          <TouchableOpacity key={key} onPress={() => onToggleMeal(key)} activeOpacity={0.7} style={[s.mealCard, active && s.mealCardActive]}>
            <Text style={s.mealEmoji}>{meta.emoji}</Text>
            <Text style={[s.mealName, active && { color: "#4ade80" }]}>{key.charAt(0).toUpperCase() + key.slice(1)}</Text>
            <Text style={s.mealTime}>{meta.time}</Text>
            <View style={[s.check, active && { backgroundColor: "#4ade80" }]}>
              <Ionicons name={active ? "checkmark" : "add"} size={12} color="#fff" />
            </View>
          </TouchableOpacity>
        );
...
const s = StyleSheet.create({
  ...
  progress: { color: "#aaa", fontSize: 12, fontFamily: "Poppins_400Regular" },
  badge:    { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20, backgroundColor: "rgba(74,222,128,0.15)", borderColor: "rgba(74,222,128,0.4)", borderWidth: 1 },
  badgeTxt: { color: "#4ade80", fontSize: 10, fontFamily: "Poppins_400Regular" },
  track:    { height: 5, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 3, marginBottom: 20, overflow: "hidden" },
  fill:     { height: 5, backgroundColor: "#4ade80", borderRadius: 3 },
  mealRow:  { flexDirection: "row", gap: 15 },
  mealCard: { flex: 1, padding: 12, borderRadius: 14, alignItems: "center", backgroundColor: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.08)", borderWidth: 1 },
  mealCardActive: { borderColor: "rgba(74,222,128,0.4)", backgroundColor: "rgba(74,222,128,0.1)" },
  mealEmoji:{ fontSize: 22, marginBottom: 4 },
  mealName: { color: "#aaa", fontSize: 12, fontFamily: "Poppins_400Regular" },
  mealTime: { color: "#777", fontSize: 9,  fontFamily: "Poppins_400Regular", marginTop: 2 },
  check:    { width: 20, height: 20, borderRadius: 10, backgroundColor: "rgba(255,255,255,0.1)", alignItems: "center", justifyContent: "center", marginTop: 8 },
});