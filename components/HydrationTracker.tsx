...
export const HydrationTracker: React.FC<Props> = ({ waterML, onAdd, onRemove, goalL }) => {
  ...
  const pct       = Math.min(liters / goalL, 1);
  const cups      = Math.round(waterML / 250);
  const totalCups = Math.round((goalL * 1000) / 250);
  ...
  return (
    <View style={s.card}>
      <View style={s.topRow}>
        ...
        <View style={[s.pctBadge, { borderColor: "rgba(74,222,128,0.4)" }]}>
          <Text style={s.pctTxt}>{pct >= 1 ? "✓ Goal met!" : `${Math.round(pct * 100)}%`}</Text>
        </View>
      </View>
      <View style={s.track}>
      ...
      <View style={s.btnRow}>
        <TouchableOpacity style={s.btn} onPress={onRemove}>
          <Ionicons name="remove" size={16} color="#4ade80" />
          <Text style={s.btnTxt}>250ml</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.btn, s.btnAdd]} onPress={onAdd}>
          <Ionicons name="add" size={16} color="#4ade80" />
          <Text style={[s.btnTxt, { color: "#4ade80" }]}>250ml</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  sub:       { color: "#aaa", fontSize: 10, fontFamily: "Poppins_400Regular" },
  pctBadge:  { paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20, backgroundColor: "rgba(74,222,128,0.15)", borderWidth: 1 },
  pctTxt:    { color: "#4ade80", fontSize: 10, fontFamily: "Poppins_400Regular" },
  track:     { height: 6, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 3, marginBottom: 15, overflow: "hidden" },
  fill:      { height: 6, backgroundColor: "#4ade80", borderRadius: 3 },
  cupsGrid:  { flexDirection: "row", flexWrap: "wrap", gap: 4, marginBottom: 20 },
  cup:       { width: 28, height: 28, borderRadius: 8, backgroundColor: "rgba(255,255,255,0.05)", alignItems: "center", justifyContent: "center" },
  cupFilled: { backgroundColor: "rgba(74,222,128,0.15)" },
  btnRow:    { flexDirection: "row", gap: 15 },
  btn:       { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, padding: 10, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.06)", borderColor: "rgba(74,222,128,0.2)", borderWidth: 1 },
  btnAdd:    { backgroundColor: "rgba(74,222,128,0.1)", borderColor: "rgba(74,222,128,0.4)" },
  btnTxt:    { color: "#999", fontSize: 12, fontFamily: "Poppins_400Regular" },
});