...
export const SleepInput: React.FC<Props> = ({ value, onChange, goal }) => {
  ...
  return (
    <View style={s.card}>
      <View style={s.goalRow}>
        <Text style={s.goalTxt}>Goal: {goal}h</Text>
        <View style={[s.goalBadge, { borderColor: value >= goal ? "#4ade80" : "rgba(74,222,128,0.4)" }]}>
          <Text style={[s.goalBadgeTxt, { color: "#4ade80" }]}>
            {value >= goal ? "✓ Reached" : `${(goal - value).toFixed(1)}h left`}
          </Text>
        </View>
      </View>
      ...
      <View style={s.track}>
        <View style={[s.fill, { width: `${Math.min((value / goal) * 100, 100)}%` }]} />
      </View>
      <View style={s.presets}>
        {presets.map(h => (
          <TouchableOpacity
            ...
            onPress={() => onChange(h)}
          >
            <Text style={[s.presetTxt, value === h && { color: "#4ade80" }]}>{h}h</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
...
const s = StyleSheet.create({
  card:         { borderRadius: 25, padding: 15, marginBottom: 30, borderColor: "rgba(74,222,128,0.3)", borderWidth: 1, overflow: "hidden", backgroundColor: "rgba(0, 26, 17, 0.50)" },
  goalRow:      { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
  goalTxt:      { color: "#aaa", fontSize: 11, fontFamily: "Poppins_400Regular" },
  goalBadge:    { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20, borderWidth: 1, backgroundColor: "rgba(74,222,128,0.1)" },
  goalBadgeTxt: { fontSize: 10, fontFamily: "Poppins_400Regular" },
  row:          { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  sub:          { color: "#999", fontSize: 10, fontFamily: "Poppins_400Regular" },
  track:        { height: 5, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 3, marginTop: 15, marginBottom: 20, overflow: "hidden" },
  ...
  presetTxt:    { color: "#999", fontSize: 11, fontFamily: "Poppins_400Regular" },
});