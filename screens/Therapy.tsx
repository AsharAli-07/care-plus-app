...
import { WATCH_STATUS_LABELS, MOVEMENT_DB_VALUES } from "../ble/constants";

const { width } = Dimensions.get("window");

const parseSessionDateTimeClient = (session_date: string, session_time: string): Date | null => {
  if (!session_date || !session_time) return null;
  const dateOnly = session_date.split(" ")[0].split("T")[0];
  const [y, m, d] = dateOnly.split("-").map(Number);
  if (!y || !m || !d) return null;

  const match = session_time.trim().match(/^(\d{1,2}):(\d{2})\s*(AM|PM)?$/i);
...
const formatSessionDate = (session_date: string): string => {
  const dateOnly = session_date.split(" ")[0].split("T")[0];
  const [y, m, d] = dateOnly.split("-").map(Number);
  if (!y || !m || !d) return session_date;
...
interface SensorData {
  ...
  temperature: string;
  hrv: number;
motionStatus: "still" | "moving" | "fall";
  lastUpdated: string;
}
...
const SessionCard = ({ session, onPress }: { session: BookedSession; onPress: () => void }) => {
  ...
  {session.therapist_name} · {formatSessionDate(session.session_date)} at {session.session_time}
</Text>
        </View>
...
const Therapy = ({ navigation }: any) => {
 ...
 const [sensorRecordedAt, setSensorRecordedAt] = useState<string | null>(null);
  ...
  const now = new Date();
  const visible = (sessionsRes.value.data || []).filter((s: BookedSession) => {
    if (s.status !== "upcoming") return true; // completed/cancelled still shown historically
    const dt = parseSessionDateTimeClient(s.session_date, s.session_time);
    return !dt || dt.getTime() >= now.getTime(); // hide upcoming sessions whose time already passed
  });
  setSessions(visible);
}
...
if (isConnected && watchData.heartRate !== "--") {
  ...
  setSensor({
    ...
    hrv: 0,
motionStatus: (MOVEMENT_DB_VALUES[watchData.watchStatus as number] ?? "still") as "still" | "moving" | "fall",
  ...
  setSensor({
    ...
    hrv: 0,
    motionStatus: (v.movement === "fall" ? "fall" : v.movement === "moving" ? "moving" : "still") as "still" | "moving" | "fall",
  ...
  const getGreeting = () => {
    if (sensor) {
if (sensor.motionStatus === "fall") return "⚠️ A fall was detected. Are you okay?";
...
<Chip
  icon={sensor.motionStatus === "fall" ? "warning-outline" : sensor.motionStatus === "moving" ? "walk-outline" : "body-outline"}
  label="Motion"
  value={sensor.motionStatus === "fall" ? "Fall!" : sensor.motionStatus === "moving" ? "Moving" : "Still"}
  alert={sensor.motionStatus === "fall"}
/>
                  {latestMood && (
...
const styles = StyleSheet.create({
  ...
  bookBtn: {
    ...
    borderColor: "rgba(74,222,128,0.3)", borderWidth: 1, paddingVertical: 12
  },
  ...
  emptyBtn: {
    ...
    width: "100%", marginTop: 15, borderColor: "rgba(74,222,128,0.3)", borderWidth: 1, paddingVertical: 12
  },
